	...
	$.widget( "ui.coverflow", {
		...
		_refresh: function( state, from, to ) {
			...
			this.items.each( function ( i ) {
				...
				var degree = mod * (side === "right" ? 20 : -20);
				// var scale = (mod === 0 ? 1 : .9);
				var scale = !mod ? 1 : .85;
				...
				// css.transform = "translate3d(0, 0px, 0px) rotateY("+degree+"deg) scale("+scale+","+scale+")";

				if (mod) {
					css.transform = (side === "right" ? 'rotateY(30deg) scale(0.35, 0.85)':'rotateY(-10deg) scale(.68,.85)');
					$(this).attr('data-side',(side === 'right' ? 'left' : 'right'));

				}
				else {
					css.transform = 'rotateY(0deg) scale(1,1)';
					$(this).attr('data-side','center');
				}
				// console.log("SIDE: "+side+" — MOD: "+mod+" STATE: "+state+'\n    '+css.transform);
